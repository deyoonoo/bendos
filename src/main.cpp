...
#if defined(NDEBUG)
#error "BDS cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ubds) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zBDS spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zBDS is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend.HasValidSignature())
                return error("%s: V2 zBDS spend does not have a valid signature\n", __func__);
        ...
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zBDS without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend.HasValidSerial(Params().Zerocoin_Params(fUseV1Params))) {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zBDS spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zBDS spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zbds");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zBDS mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight <= 1) {
        ...
        nSubsidy = 0.5 * COIN;
    } else if (nHeight > 999 && nHeight <= 45000 ) { // Last POW Block 400
        nSubsidy = 1 * COIN;
    } else if (nHeight > 45000 && nHeight <= 90000 ) {
        nSubsidy = 1.5 * COIN;
    } else if (nHeight > 90000 && nHeight <= 135000) {
        nSubsidy = 2 * COIN;
    } else if (nHeight > 135000 && nHeight <= 180000 ) { // Last block for fork height to new block reward - Approx 27.12.2018 1PM GMT +1
        nSubsidy = 2.5 * COIN;
    } else if (nHeight > 180000 && nHeight <= 225000 ) {
        nSubsidy = 3 * COIN;
    } else if (nHeight > 225000 && nHeight <= 270000 ) {
        nSubsidy = 3.5 * COIN;
    } else if (nHeight > 270000 && nHeight <= 315000 ) {
        nSubsidy = 4 * COIN;
    } else if (nHeight > 315000 && nHeight <= 360000 ) {
        nSubsidy = 4.5 * COIN;
    } else if (nHeight > 360000 && nHeight <= 405000 ) {
        nSubsidy = 5 * COIN;
    } else if (nHeight > 405000 && nHeight <= 450000 ) {
        nSubsidy = 5.5 * COIN;
    } else if (nHeight > 450000 && nHeight <= 495000 ) {
        nSubsidy = 6 * COIN;
    } else if (nHeight > 495000 && nHeight <= 540000 ) {
        nSubsidy = 6.5 * COIN;
    } else if (nHeight > 540000 && nHeight <= 585000 ) {
        nSubsidy = 7 * COIN;
    } else if (nHeight > 585000 && nHeight <= 630000 ) {
        nSubsidy = 7.5 * COIN;
    } else if (nHeight > 630000 && nHeight <= 675000 ) {
        nSubsidy = 8 * COIN;
    } else if (nHeight > 675000 && nHeight <= 720000 ) {
        nSubsidy = 8.5 * COIN;
    } else if (nHeight > 720000 && nHeight <= 765000 ) {
        nSubsidy = 9 * COIN;
    } else if (nHeight > 765000 && nHeight <= 810000 ) {
        nSubsidy = 9.5 * COIN;
    } else if (nHeight > 810000 && nHeight <= 855000 ) {
        nSubsidy = 10 * COIN;
    } else if (nHeight > 855000 && nHeight <= 4044522 ) {
        nSubsidy = 5 * COIN;
    } else if (nHeight > 4044522) {
        nSubsidy = 0 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZBDSStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from BDS
...
void RecalculateZBDSMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating minted ZBDS..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZBDS..."), percent);
        }
...
void RecalculateZBDSSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZBDS..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZBDS..."), percent);
        }
        ...
        //Remove spends from zBDS supply
...
bool RecalculateBDSSupply(int nHeightStart)
{
    ...
    if (nHeightStart == Params().Zerocoin_StartHeight())
        nSupplyPrev = pindex->pprev->nMoneySupply;

    uiInterface.ShowProgress(_("Recalculating BDS supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating BDS supply..."), percent);
        }
...
{
    // BDS: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZBDSSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zBDS supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZBDSMinted();
        RecalculateZBDSSpent();
        RecalculateBDSSupply(Params().Zerocoin_StartHeight());
    }

    //Track zBDS money supply in the block index
    if (!UpdateZBDSSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zBDS supply for block=%s height=%d", __func__,
...
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert BDS to zBDS
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zBDS spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zBDS serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if ((Params().NetworkID() != CBaseChainParams::REGTEST) && block.IsProofOfWork() && (pindexPrev->nHeight + 1 <= 0)) {
    ...
    if (block.nBits != nBitsRequired && block.nVersion >= 5)
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nVersion >= 5 &&
        CBlockIndex::IsSuperMajority(5, pindexPrev, Params().EnforceBlockUpgradeMajority())) {
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    ...
    if (CZBDSStake* zBDS = dynamic_cast<CZBDSStake*>(stake)) {
        ...
        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zBDS stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zBDS->GetValue()));
        if (nChecksum200 != zBDS->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zBDS->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZBDS() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zBDS fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> bdsInputs;
        std::vector<CTxIn> zBDSInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zBDSInputs.push_back(stakeIn);
            }else{
                bdsInputs.push_back(stakeIn);
            }
        }
        const bool hasBDSInputs = !bdsInputs.empty();
        const bool hasZBDSInputs = !zBDSInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasBDSInputs)
                    ...
                    for (const CTxIn& bdsIn : bdsInputs){
                        if(bdsIn.prevout == in.prevout){
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction &t : bl.vtx) {
                    for (const CTxIn &in: t.vin) {
                        ...
                        for (const CTxIn &stakeIn : bdsInputs) {
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zBDS mints and %d zBDS spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // BDS: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zbds", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zbds", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
