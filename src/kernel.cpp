...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZBDSStake(spend));
    } else {
        ...
        CBDSStake* bdsInput = new CBDSStake();
        bdsInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(bdsInput);
    }
