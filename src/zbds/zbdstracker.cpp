...
CzBDSTracker::CzBDSTracker(std::string strWalletFile)
{
...
CzBDSTracker::~CzBDSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBDSTracker::Init()
{
...
bool CzBDSTracker::Archive(CMintMeta& meta)
{
...
bool CzBDSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBDSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBDSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBDSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzBDSTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzBDSTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzBDSTracker::GetSerialHashes()
{
...
CAmount CzBDSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBDSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBDSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBDSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBDSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBDSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBDSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBDSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBDSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBDSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBDSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBDSWallet* zBDSWallet)
{
    bool iszBDSWalletInitialized = (NULL != zBDSWallet);
    CMintMeta meta;
    ...
    if (! iszBDSWalletInitialized)
        zBDSWallet = new CzBDSWallet(strWalletFile);
    meta.isSeedCorrect = zBDSWallet->CheckSeed(dMint);
    if (! iszBDSWalletInitialized)
        delete zBDSWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBDSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBDSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBDSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBDSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBDSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBDSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBDSWallet* zBDSWallet = new CzBDSWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zBDSWallet);
        }
        delete zBDSWallet;
        LogPrint("zero", "%s: added %d dzbds from DB\n", __func__, listDeterministicDB.size());
    }
