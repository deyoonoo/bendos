...
CzBDSWallet::CzBDSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbds seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBDSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBDSSeed_deprecated()) {
                    LogPrintf("%s: Updated zBDS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbds seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbds wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBDSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBDSCount(nCountLastUsed);
    else if (!walletdb.ReadZBDSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBDSWallet::Lock()
{
    seedMaster = 0;
}

void CzBDSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBDSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBDS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBDSWallet::LoadMintPoolFromDB()
{
...
void CzBDSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBDSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBDSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbdsTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBDSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBDS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbdsTracker which also adds to database
    pwalletMain->zbdsTracker->Add(dMint, true);
...
void CzBDSWallet::SeedToZBDS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBDSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBDSWallet::GenerateDeterministicZBDS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBDSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBDSWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBDSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
