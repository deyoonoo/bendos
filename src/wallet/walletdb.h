...
public:
    ...
    bool WriteZBDSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBDSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBDSSeed_deprecated(uint256& seed);
    bool EraseZBDSSeed();
    bool EraseZBDSSeed_deprecated();

    bool WriteZBDSCount(const uint32_t& nCount);
    bool ReadZBDSCount(uint32_t& nCount);
