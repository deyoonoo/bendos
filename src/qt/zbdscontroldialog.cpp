...
#include "ui_zbdscontroldialog.h"
...
void ZPnyControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zbdsTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zbdsTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zBDS.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBDS seed used to mint this zBDS is not the same as currently hold in the wallet";
            else
