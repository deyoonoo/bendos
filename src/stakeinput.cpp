...
bool CZPnyStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
    ...
    if (!pwallet->CreateZBDSOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBDS output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBDS", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBDSOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBDS output", __func__);
...
bool CZPnyStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    ...
    CMintMeta meta;
    if (!zbdsTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zbdsTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
