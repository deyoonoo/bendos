...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZBDSStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    ...
    if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZBDSStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZBDSStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
    ...
    CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight, blockValue, 0, fZBDSStake);
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    CAmount nReward2 = GetBlockValue(nBlockHeight-2);
    ...
    CAmount requiredMasternodePayment2 = GetMasternodePayment(nBlockHeight-2, nReward2, nMasternode_Drift_Count, txNew.IsZerocoinSpend());
    ...
    BOOST_FOREACH (CMasternodePayee& payee, vecPayments) {
        ...
        BOOST_FOREACH (CTxOut out, txNew.vout) {
            if (payee.scriptPubKey == out.scriptPubKey) {
                if(out.nValue >= requiredMasternodePayment || out.nValue >= requiredMasternodePayment2)
                    found = true;
                else
...
bool CMasternodePaymentWinner::IsValid(CNode* pnode, std::string& strError)
{
    ...
    if (n > MNPAYMENTS_SIGNATURES_TOTAL) {
        ...
        if (n > MNPAYMENTS_SIGNATURES_TOTAL + 1) {
            strError = strprintf("Masternode not in the top %d (%d)", MNPAYMENTS_SIGNATURES_TOTAL + 1, n);
            LogPrint("masternode","CMasternodePaymentWinner::IsValid - %s\n", strError);
