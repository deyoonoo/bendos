...
*Setup instructions for a gitian build of BDS using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BDS
...
Clone the git repositories for bds and gitian and then checkout the bds version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bds-crypto/bds.git
cd bds
...
Follow the instructions in [doc/release-process.md](release-process.md) in the bds repository
...
To build BDS (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the bds repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bds/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/bds-crypto/bds
...
For example:
```bash
URL=https://github.com/crowning-/bds.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit bds=${COMMIT} --url bds=${URL} ../bds/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bds=${COMMIT} --url bds=${URL} ../bds/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bds=${COMMIT} --url bds=${URL} ../bds/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bds-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bds-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/bds-build.assert
```
...
[bds/gitian.sigs](https://github.com/bds-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
